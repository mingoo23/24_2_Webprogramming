$(document).ready(function () {
    ...
    function loadContent(file) {
        ...
        if (cache[file]) {
            ...
            $("#workspace-content").empty().html(cache[file]).find(".workspace-tab-content").addClass("active");
        } else {
            // AJAX 요청으로 데이터 가져오기
            ...
            $("#workspace-content").load(file, function (response, status, xhr) {
                if (status === "success") {
                    ...
                    cache[file] = response;
                    // 기존 콘텐츠를 비활성화하고 새 콘텐츠 삽입
                    $("#workspace-content").empty().html(response).find(".workspace-tab-content").addClass("active");
                } else if (status === "error") {
                    console.error("콘텐츠를 로드하는 중 오류 발생:", xhr.status, xhr.statusText);
                    $("#workspace-content").html("<p>콘텐츠를 불러오지 못했습니다. 잠시 후 다시 시도해주세요.</p>");
                }
            });
        }
    }
